...
def help():
    ...
    PIPELINE FLOW
    ------------------------------------------------------------
    1) Load YOLOv5 segmentation model
    2) Read frames from source (video / webcam / images)
    3) Run detection + segmentation masks
    4) Track objects using SORT (unique IDs)
    5) Track centroid movement across a vertical line
    6) Count objects crossing the line (once per ID)
    7) Save annotated video + CSV count log

    ------------------------------------------------------------
    COUNTING LOGIC
    ------------------------------------------------------------
    • A vertical counting line is defined at:
        LINE_X = 400

    • Each object has a centroid (cx, cy)

    • Crossing condition:
        If centroid moves from LEFT → RIGHT
        OR RIGHT → LEFT across LINE_X

    • Each object ID is counted ONLY ONCE:
        counted_ids = set()

    • Prevents double counting even if the object reappears

    ------------------------------------------------------------
    TRACKING (SORT)
    ------------------------------------------------------------
    • SORT assigns a unique track_id to each object
    • Bounding boxes are matched using IoU
    • Class name is assigned by best IoU match
    • Tracker maintains identity across frames

    ------------------------------------------------------------
    SEGMENTATION
    ------------------------------------------------------------
    • YOLOv5-Seg model outputs:
        - Bounding boxes
        - Class IDs
        - Segmentation masks

    • Masks are rendered as semi-transparent overlays
    • Bounding boxes and class labels are drawn on top
    ...
    ------------------------------------------------------------
    Saved under:
        runs/seg_count/<experiment_name>/

    Files generated:
        output.mp4     → Annotated output video
        counts.csv     → Frame-wise object counts

    CSV FORMAT:
        time, frame, <class_1>, <class_2>, ...
    ...
    • Video writer is safely released
    • CSV file is properly closed
