...
#     """Return predefined color or deterministic random color for unknown classes."""
#     if cls_name in CLASS_COLORS:
#         return CLASS_COLORS[cls_name]
#     # Unknown class → unique deterministic color based on class name
#     np.random.seed(abs(hash(cls_name)) % (2**32))
#     return tuple(int(c) for c in np.random.randint(40, 255, 3))

# # ================= OCR CONFIG =================
# # Predefined label texts — OCR match required to count under label OCR section
...
# def compute_overlaps(tracks, counted_track_ids):
#     """
#     Check every pair of tracked boxes.
#     If IoU > threshold → treat as overlap.
#     Returns set of track_ids that are overlapping.
#     """
#     OVERLAP_IOU_THRESHOLD = 0.05   # tune this — even slight overlap triggers it
...
#     COUNTING RULES
#     ──────────────
#     label class  → count fires ONLY when OCR matches a predefined text (once per track_id)
#     all others   → count fires on first detection of that track_id (once per track_id)
#     unknown class → same as above, auto-registered dynamically
#     """

#     # ── label OCR counts (predefined text → int) ──
#     track_ocr_text  = {}   # track_id → OCR result dict
#     ocr_text_counts = {}   # count_key (UN code / matched text) → int

#     # ── per-class object counts (once per unique track_id) ──
#     # track_id → cls_name  — used to ensure each object counted exactly once
#     counted_track_ids = {}
#     # cls_name → int  — dynamically grows for unknown classes too
#     class_counts = {}
...
#                 masks_raw    = process_mask(proto[0], mask_coeffs, det[:, :4],
...
#                 # ── LABEL CLASS ──
#                 # Count only fires when OCR reads a predefined text (once per track_id)
...
#                         if un_code or matched_key:
#                             # ── predefined text found → store OCR result ──
...
#                             }
#                             # OCR count — once per track_id, predefined text only
#                             count_key = un_code if un_code else matched_key
#                             ocr_text_counts[count_key] = ocr_text_counts.get(count_key, 0) + 1

#                             # Also bump class_counts for label (predefined match)
#                             if track_id not in counted_track_ids:
#                                 counted_track_ids[track_id] = cls_name
#                                 class_counts[cls_name] = class_counts.get(cls_name, 0) + 1

#                         # If OCR ran but NO predefined text matched → do NOT count yet
#                         # track_ocr_text stays empty for this track_id so OCR retries next frame
#                         # (until predefined match found or track dies)

#             else:
#                 # ── ALL OTHER CLASSES (predefined or unknown) ──
#                 # Count fires on first time this track_id is seen, regardless of OCR
#                 if track_id not in counted_track_ids:
#                     counted_track_ids[track_id] = cls_name
#                     # Auto-register any new/unknown class dynamically
#                     class_counts[cls_name] = class_counts.get(cls_name, 0) + 1
...
#         # Section 1: CLASS COUNTS  (all classes, unique per track_id)
#         # Section 2: LABEL OCR     (label class only, predefined text only)
#         # =============================================
#         y_offset = 20

#         # ── Section 1: CLASS COUNTS ──
#         draw_text_with_border(frame, "=== CLASS COUNTS ===", (15, y_offset),
...
#                 border_color=cls_color
#             )
#             y_offset += 20

#         y_offset += 8

#         # ── Section 2: LABEL OCR COUNTS ──
#         draw_text_with_border(frame, "=== LABEL OCR ===", (15, y_offset),
#                               text_color=(0, 215, 255))
#         y_offset += 22
...
#                 y_offset += 20
#         else:
#             draw_text_with_border(frame, "no labels read yet", (15, y_offset),
#                                   text_color=(80, 80, 80))
...
# # append BOTH so models/, utils/, sort/ are all found
...
#         gray = cv2.resize(gray, None, fx=2, fy=2, interpolation=cv2.INTER_CUBIC)
#         # FIXED
#         results = ocr_reader.readtext(gray, detail=1, paragraph=False)  # paragraph=False keeps bbox
#         texts = [t for _, t, c in results if c > 0.3]
...
#     def looped_dataset(source, img_size, stride):
#         while True:
#             yield from LoadImages(source, img_size=img_size, stride=stride)

#     dataset = LoadStreams(source, img_size=imgsz, stride=stride) \
#         if source.isnumeric() else looped_dataset(source, img_size=imgsz, stride=stride)
...
#         # Compute IoU between every pair of tracked boxes
#         # No need for a trained 'overlap' class — calculated automatically
...
#                     best_iou = iou
#                     best_det = d
#                     cls_name = names.get(d[5], f"class_{d[5]}") 
...
#             # ===== OVERLAP — auto detected via IoU, NOT model class =====
#             if is_overlapping:
#                 cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 0, 255), 3)
#                 cv2.putText(frame, "! OVERLAP", (x1, y1 - 25),
#                             cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
...
#                                 counted_track_ids[track_id] = cls_name
#                                 class_counts[cls_name] = class_counts.get(cls_name, 0) + 1
#                                 # If overlapping at the moment of first count
#                                 if is_overlapping:
#                                     class_counts["overlap"] = class_counts.get("overlap", 0) + 1

#                         # If OCR ran but NO predefined text matched → do NOT count yet
#                         # track_ocr_text stays empty → OCR retries next frame
...
#                     # Auto-register any new/unknown class dynamically
#                     class_counts[cls_name] = class_counts.get(cls_name, 0) + 1
#                     # Count overlap separately if overlapping at first detection
#                     if is_overlapping:
#                         class_counts["overlap"] = class_counts.get("overlap", 0) + 1
...
#             out_path = rf"D:\bhanu\packaging\crown\engineering\ai_infernces\output_{timestamp}.mp4"
...
#         # after model.warmup — get total frame count
#         total_frames = int(vid_cap.get(cv2.CAP_PROP_FRAME_COUNT)) if vid_cap else 0
#         frame_num = 0
#     # ← loop ends here
#     if out_writer:
#         out_writer.release()
#         print("✅ Output saved to: output_result.mp4")
...
def run(
    ...
    # FIXED — parameter name matches keyword argument
    def looped_dataset(src, img_size, stride):
        while True:
            yield from LoadImages(src, img_size=img_size, stride=stride)
    ...
    for data in tqdm(dataset, unit="frame"):
        ...
        # get total frame count once on first frame
        if frame_num == 0 and vid_cap is not None:
                ...
                if track_id not in track_ocr_text and best_det is not None:
                    ...
                    if crop.size:
                        ...
                        if un_code or matched_key:
                            ...
                            # REPLACE WITH — use the human-readable detected text as the key
                            count_key = display_text   # e.g. "UN3480", "FRAGILE", "LITHIUM ION BATTERIES"
                            ocr_text_counts[count_key] = ocr_text_counts.get(count_key, 0) + 1
        ...
        for cls_key, cnt in sorted(class_counts.items()):
            cls_color = get_class_color(cls_key)
            draw_text_with_border(
                frame, f"{cls_key:<12}: {cnt}", (15, y_offset),
                text_color=cls_color, border_color=cls_color
            )
            y_offset += 20
        ...
        if ocr_text_counts:
            for detected_text, cnt in sorted(ocr_text_counts.items()):
                draw_text_with_border(
                    frame, f"{detected_text} : {cnt}", (15, y_offset),
                    text_color=(0, 255, 255)
                )
                y_offset += 20
        else:
        ...
        print(
            f"\r  Frame {frame_num}/{total_frames} ({pct:.1f}%)  |  "
            f"FPS: {fps_live:.1f}  |  "
            f"Elapsed: {elapsed:.0f}s  |  "
            f"Tracks: {len(tracks)}  |  "
            f"Labels OCR'd: {len(track_ocr_text)}",
            end="", flush=True
        )
